...
/*************************************************************************************
      ...
      * 类 名 称:   PhoneticHelper
      * 机器名称:   LVJUNLEI-PC
      * 命名空间:   CommonLibrary.Helpers
      * 文 件 名:   PhoneticHelper
      * 创建时间:   16.5.12 14:41:37
      * 作   者:    LvJunlei
...
namespace CommonLibrary.Helpers
{
    public class PhoneticHelper
    {
        ...
        private static readonly string[] OtherChinese = {
            ...
            ,"黟","黢","黩","黧","黥","黪","黯","鼢","鼬","鼯","鼹","鼷","鼽","鼾","齄","堃"
        };
        ...
        private static readonly string[] OtherPinYin = {
               "Chu","Ji","Wu","Gai","Nian","Sa","Pi","Gen","Cheng","Ge","Nao","E","Shu","Yu","Pie","Bi",
                ...
                "Xia","Yi","Qu","Du","Li","Qing","Can","An","Fen","You","Wu","Yan","Xi","Qiu","Han","Zha","Kun"
           };
        #endregion 二级汉字
        ...
        /// <returns></returns>        
        public static string GetPhoneticFirstLetter(char chineseCh)
        {
            var rs = GetPhonetic(chineseCh);
        ...
        /// <returns></returns>
        public static string GetPhoneticFirstLetters(string chineseStr, string splitChar = "")
        {
            if (string.IsNullOrEmpty(chineseStr)) return string.Empty;

            var sb = new StringBuilder(chineseStr.Length + 1);
            var chs = chineseStr.ToCharArray();

            foreach (var t in chs)
            {
                sb.AppendFormat("{0}{1}", GetPhoneticFirstLetter(t), splitChar);
            }
        ...
        /// <param name="chineseCh"></param>
        /// <returns></returns>
        public static string GetPhonetic(char chineseCh)
        {
            // 拉丁字符            
            if (chineseCh <= '\x00FF') return chineseCh.ToString();

            // 标点符号、分隔符            
            if (char.IsPunctuation(chineseCh) || char.IsSeparator(chineseCh)) return chineseCh.ToString();

            // 非中文字符            
            if (chineseCh < '\x4E00' || chineseCh > '\x9FA5') return chineseCh.ToString();

            var arr = Encoding.GetEncoding("gb2312").GetBytes(chineseCh.ToString());
            //Encoding.Default默认在中文环境里虽是GB2312，但在多变的环境可能是其它
            //var arr = Encoding.Default.GetBytes(chineseCh.ToString()); 
            var chr = arr[0] * 256 + arr[1] - 65536;

            //***// 单字符--英文或半角字符  
            if (chr > 0 && chr < 160) return chineseCh.ToString();
            #region 中文字符处理

            // 判断是否超过GB2312-80标准中的汉字范围
            if (chr > LastChCode || chr < FirstChCode)
            {
                return chineseCh.ToString();
            }
            // 如果是在一级汉字中
            ...
            {
                var pos = Array.IndexOf(OtherChinese, chineseCh.ToString());
        ...
        public static string GetPhonetics(string chineseStr, string splitChar = "")
        {
            if (string.IsNullOrEmpty(chineseStr)) return string.Empty;

            var sb = new StringBuilder(chineseStr.Length * 10);
            var chs = chineseStr.ToCharArray();

            foreach (var t in chs)
            {
                sb.AppendFormat("{0}{1}", GetPhonetic(t), splitChar);
            }
